...
const TextualHistory = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(activitiesRef, (snapshot) => {
      ...
      if (data['mcq-challenges']) {
        Object.entries(data['mcq-challenges']).forEach(([key, value]) => {
          Object.values(value).forEach(challenge => {
            allActivities.push({
              id: `mcq-${key}`,
              type: 'mcq',
              ...
              chips: [`Difficulty: ${challenge.difficulty}`, `Questions: ${challenge.attemptedQuestions}`]
            });
          });
        });
      }
